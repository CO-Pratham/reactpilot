...
function findReleaseCatalog(startDir: string): ReleaseCatalog | null {
  ...
  // Fallback: check relative to the CLI's installation folder (dist/index.cjs)
  try {
    const fallbackPath = path.join(__dirname, 'releases.json');
    if (fs.existsSync(fallbackPath)) {
      return JSON.parse(fs.readFileSync(fallbackPath, 'utf-8')) as ReleaseCatalog;
    }
  } catch {
